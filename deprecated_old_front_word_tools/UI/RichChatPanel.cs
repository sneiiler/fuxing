...
namespace FuXing.UI
{
    ...
    {
        int MeasureHeight(Canvas g, int width);
        void Paint(Canvas g, Rectangle bounds, bool isUser);
    ...
    internal class TextBlock : IContentBlock
    {
        ...
        public int MeasureHeight(Canvas g, int width)
        {
            if (string.IsNullOrEmpty(Text)) return 0;
            int totalH = 0;
            foreach (var seg in GetSegments())
            {
                var size = g.MeasureText(seg.Text, seg.Font, width, FormatFlags.Top | FormatFlags.Left);
                totalH += size.Height + seg.ExtraSpacing;
            }
            return Math.Max(18, totalH);
        }

        public void Paint(Canvas g, Rectangle bounds, bool isUser)
        {
            ...
            foreach (var seg in GetSegments())
            {
                ...
                var size = g.MeasureText(seg.Text, seg.Font, bounds.Width, FormatFlags.Top | FormatFlags.Left);
                g.DrawText(seg.Text, seg.Font, color,
    ...
    internal class BlockquoteBlock : IContentBlock
    {
        ...
        {
            if (string.IsNullOrEmpty(Text)) return 0;
            var size = g.MeasureText(Text, QuoteFont, width - PadLeft - 10, FormatFlags.Top | FormatFlags.Left);
        ...
        public void Paint(Canvas g, Rectangle bounds, bool isUser)
        {
            ...
            // 圆角背景
            using (var path = GdiHelper.RoundedRect(bounds, Radius))
                g.Fill(QuoteBgColor, path);

            // 左侧蓝色竖线
            g.DrawLine(QuoteBorderColor, LeftBarWidth,
                bounds.X + LeftBarWidth / 2 + 1, bounds.Y + 4,
            ...
            g.DrawText(Text, QuoteFont, QuoteFgColor, textRect,
                FormatFlags.Top | FormatFlags.Left);
        }
    ...
    internal class FencedCodeBlock : IContentBlock
    {
        ...
        public int MeasureHeight(Canvas g, int width)
        {
            ...
            var size = g.MeasureText(Code, CodeFont, width - PadX * 2, FormatFlags.Top | FormatFlags.Left);
        ...
        public void Paint(Canvas g, Rectangle bounds, bool isUser)
        {
            ...
            {
                g.Fill(CodeBg, path);
                g.Draw(CodeBorder, 1f, path);
            }

            // 语言标签
            if (!string.IsNullOrEmpty(Language))
            {
                g.DrawText(Language, LangFont, LangFg,
                    new Rectangle(bounds.X + PadX, bounds.Y + 4, bounds.Width - PadX * 2, 16),
                    FormatFlags.Top | FormatFlags.Left);
            }

            // 代码文本
            g.DrawText(Code, CodeFont, CodeFg,
    ...
    internal class TableBlock : IContentBlock
    {
        ...
        private int[] CalcColumnWidths(Canvas g, int availableWidth)
        {
            ...
            for (int r = 0; r < _rows.Length; r++)
            {
                ...
                {
                    var size = g.MeasureText(_rows[r][c], font, 9999);
        ...
        private int RowHeight(Canvas g, int[] colWidths, int rowIdx)
        {
            ...
            for (int c = 0; c < colWidths.Length && c < row.Length; c++)
            {
                ...
                var size = g.MeasureText(row[c], font, textW, FormatFlags.Top | FormatFlags.Left);
        ...
        public void Paint(Canvas g, Rectangle bounds, bool isUser)
        {
            ...
            var gridColor = isUser ? Color.FromArgb(80, 255, 255, 255) : GridColor;
            ...
            for (int r = 0; r < _rows.Length; r++)
            {
                ...
                // 行背景
                if (r == 0)
                    g.Fill(headerBg, new Rectangle(x, y, tableWidth, rowH));
                else if (r % 2 == 0)
                    g.Fill(altBg, new Rectangle(x, y, tableWidth, rowH));
                ...
                for (int c = 0; c < colWidths.Length; c++)
                {
                    ...
                    g.DrawText(text, font, fgColor, cellRect,
                        FormatFlags.VerticalCenter | FormatFlags.Left);
                    // 右侧竖线
                    g.DrawLine(gridColor, 1f, x + colWidths[c], y, x + colWidths[c], y + rowH);
                    x += colWidths[c];
                }

                // 左侧竖线
                g.DrawLine(gridColor, 1f, bounds.X, y, bounds.X, y + rowH);
                // 行底部横线
                g.DrawLine(gridColor, 1f, bounds.X, y + rowH, bounds.X + tableWidth, y + rowH);
                // 首行顶部横线
                if (r == 0)
                    g.DrawLine(gridColor, 1f, bounds.X, y, bounds.X + tableWidth, y);
    ...
    public class ThinkBlock : IContentBlock
    {
        ...
        public int MeasureHeight(Canvas g, int width)
        {
            ...
            var size = g.MeasureText(Text ?? "", ContentFont, width - 20, FormatFlags.Top | FormatFlags.Left);
        ...
        {
            // 圆角背景
            using (var path = GdiHelper.RoundedRect(bounds, 8))
            {
                g.Fill(HeaderBg, path);
                g.Draw(BorderColor, 1f, path);
            }

            // 左侧装饰线
            g.DrawLine(Color.FromArgb(156, 163, 175), 2f,
                bounds.X + 4, bounds.Y + 8, bounds.X + 4, bounds.Bottom - 8);

            // 头部: 💭 emoji + 文字 + 展开/折叠指示
            string chevron = IsExpanded ? "▾" : "▸";
            string headerText = $"{chevron}  💭 思考过程";
            g.DrawText(headerText, HeaderFont, HeaderFg,
            ...
            if (!IsExpanded)
            {
                using (var hintFont = new Font("Microsoft YaHei UI", 7.5F))
                    g.DrawText("点击展开", hintFont, Color.FromArgb(156, 163, 175),
                        new Rectangle(bounds.X + 12, bounds.Y, bounds.Width - 24, HeaderHeight),
            ...
            if (IsExpanded && !string.IsNullOrEmpty(Text))
            {
                ...
                g.DrawLine(BorderColor, 1f, bounds.X + 12, sepY, bounds.Right - 12, sepY);

                var contentRect = new Rectangle(bounds.X + 12, sepY + 4, bounds.Width - 24, bounds.Bottom - sepY - 8);
                g.DrawText(Text, ContentFont, ContentFg, contentRect,
                    FormatFlags.Top | FormatFlags.Left);
            }
        }
    ...
    public class ToolCallCard : IContentBlock
    {
        ...
        public int MeasureHeight(Canvas g, int width)
        {
            ...
            {
                h += 4; // 间距
                var codeSize = g.MeasureText(CodeSnippet, CodeFont, contentW - 16, FormatFlags.Top | FormatFlags.Left);
            ...
            {
                var size = g.MeasureText(StatusText, StatusFont, contentW, FormatFlags.Top | FormatFlags.Left);
        ...
        public void Paint(Canvas g, Rectangle bounds, bool isUser)
        {
            ...
            int textY = bounds.Y;
            g.DrawText($"{chevron}  {icon} {ToolName}", NameFont, HeaderFg,
                new Rectangle(textX, textY, bounds.Width - 24, HeaderH),
                FormatFlags.VerticalCenter | FormatFlags.Left);

            // 折叠/展开提示
            using (var hintFont = new Font("Microsoft YaHei UI", 7.5F))
                g.DrawText(IsExpanded ? "点击收起" : "点击展开", hintFont, Color.FromArgb(156, 163, 175),
                    new Rectangle(textX, textY, bounds.Width - 24, HeaderH),
                    FormatFlags.VerticalCenter | FormatFlags.Right);
            ...
            // 分隔线
            g.DrawLine(CardBorder, 1f, textX, textY, bounds.Right - 12, textY);
            ...
            if (!string.IsNullOrEmpty(CodeSnippet))
            {
                ...
                using (var codePath = GdiHelper.RoundedRect(codeRect, 6))
                    g.Fill(CodeBg, codePath);

                // 代码文本
                g.DrawText(CodeSnippet, CodeFont, CodeFg,
            ...
            if (!string.IsNullOrEmpty(StatusText))
            {
                var statusSize = g.MeasureText(StatusText, StatusFont, bounds.Width - 24, FormatFlags.Top | FormatFlags.Left);
                int statusH = Math.Max(DetailLineH, statusSize.Height);
                g.DrawText(StatusText, StatusFont, accentColor,
            ...
            {
                g.DrawText($"  ▪  {detail}", DetailFont, ContentFg,
    ...
    public class SubAgentBlock : IContentBlock
    {
        ...
        public void Paint(Canvas g, Rectangle bounds, bool isUser)
        {
            ...
            g.DrawText($"{chevron}  🤖 子智能体：{TaskName}", HeaderFont, HeaderFg,
                new Rectangle(bounds.X + 12, bounds.Y, bounds.Width - 24, HeaderH),
                FormatFlags.VerticalCenter | FormatFlags.Left);

            g.DrawText($"{statusIcon} {statusLabel}", HeaderFont,
            ...
            g.DrawLine(CardBorder, 1f, bounds.X + 12, sepY, bounds.Right - 12, sepY);
            ...
            foreach (var step in _steps)
            {
                ...
                switch (step.Type)
                {
                    case SubAgentStepType.Thinking:
                        if (step.Text == "⏳")
                            g.DrawText("💭 正在思考...", StepFont, RunningColor, rect,
                                FormatFlags.VerticalCenter | FormatFlags.Left);
                        else
                            g.DrawText($"💭 {step.Text}", StepFont, ThinkFg, rect,
                                FormatFlags.VerticalCenter | FormatFlags.Left);
                        break;
                    ...
                    case SubAgentStepType.ToolCall:
                        ...
                        g.DrawText($"🔧 {step.Text}", StepFont, ContentFg, rect,
                            FormatFlags.VerticalCenter | FormatFlags.Left);
                        g.DrawText(toolIcon, StepFont, toolColor, rect,
                            FormatFlags.VerticalCenter | FormatFlags.Right);
                        break;

                    case SubAgentStepType.Output:
                        var outputColor = step.Status == ToolCallStatus.Success ? SuccessColor : ErrorColor;
                        g.DrawText($"📋 {step.Text}", StepFont, outputColor, rect,
                            FormatFlags.VerticalCenter | FormatFlags.Left);
                        break;
                }
    ...
    internal class ApprovalCard : IContentBlock
    {
        ...
        public int MeasureHeight(Canvas g, int width)
        {
            ...
            // 描述提示语
            var descSize = g.MeasureText("AI 助手请求执行以下操作，请确认是否允许：", DescFont, textW, FormatFlags.Top | FormatFlags.Left);
            ...
            if (!string.IsNullOrEmpty(Summary))
            {
                ...
                var codeSize = g.MeasureText(Summary, CodeFont, codeW, FormatFlags.Top | FormatFlags.Left);
        ...
        public void Paint(Canvas g, Rectangle bounds, bool isUser)
        {
            ...
            // 标题
            g.DrawText("操作确认", HeaderFont, HeaderFg,
                new Rectangle(textX, y, textW, HeaderH),
                FormatFlags.VerticalCenter | FormatFlags.Left);
            y += HeaderH + SepGap;

            // 分隔线
            g.DrawLine(_result == null ? SepColor : borderColor, 1f,
                textX, y, bounds.Right - SidePad, y);
            y += 1 + ContentGap;

            // 描述提示
            string desc = "AI 助手请求执行以下操作，请确认是否允许：";
            var descSize = g.MeasureText(desc, DescFont, textW, FormatFlags.Top | FormatFlags.Left);
            g.DrawText(desc, DescFont, DescFg,
            ...
            // 工具名称徽章
            string toolLabel = "工具: ";
            var toolLabelSize = g.MeasureText(toolLabel, LabelFont, textW, FormatFlags.VerticalCenter | FormatFlags.Left);
            g.DrawText(toolLabel, LabelFont, HeaderFg,
            ...
            var toolNameSize = g.MeasureText(toolName, DescFont, textW, FormatFlags.Top | FormatFlags.Left);
            ...
            using (var path = GdiHelper.RoundedRect(badgeRect, 4))
            {
                g.Fill(ToolBadgeBg, path);
                g.Draw(ToolBadgeBorder, 1f, path);
            }
            g.DrawText(toolName, DescFont, ToolBadgeFg, badgeRect, FormatFlags.Center);
            y += ToolBadgeH + 10;
            ...
            {
                // "参数:" 标签
                g.DrawText("参数:", LabelFont, HeaderFg,
                ...
                using (var path = GdiHelper.RoundedRect(codeRect, 6))
                {
                    g.Fill(CodeBlockBg, path);
                    g.Draw(CodeBlockBorder, 1f, path);
                }
                g.DrawText(Summary, CodeFont, CodeFg,
            ...
            if (_result == null)
            {
                ...
                using (var path = GdiHelper.RoundedRect(approveRect, 6))
                {
                    g.Fill(ApproveBtnBg, path);
                    g.Draw(ApproveBtnBorder, 1f, path);
                }
                g.DrawText("允许执行", ButtonFont, ApproveBtnText, approveRect,
                    FormatFlags.Center);

                // 拒绝按钮
                var rejectRect = new Rectangle(btnX + ButtonW + ButtonGap, y, ButtonW, ButtonH);
                using (var path = GdiHelper.RoundedRect(rejectRect, 6))
                {
                    g.Fill(RejectBtnBg, path);
                    g.Draw(RejectBtnBorder, 1f, path);
                }
                g.DrawText("拒绝", ButtonFont, RejectBtnText, rejectRect,
                    FormatFlags.Center);
            }
            else
            {
                ...
                g.DrawText(resultText, ResultFont, resultColor,
    ...
    internal class AskUserCard : IContentBlock
    {
        ...
        public int MeasureHeight(Canvas g, int width)
        {
            ...
            // 问题文本
            var qSize = g.MeasureText(Question, QuestionFont, textW, FormatFlags.Top | FormatFlags.Left);
        ...
        public void Paint(Canvas g, Rectangle bounds, bool isUser)
        {
            ...
            // 左侧装饰线
            g.DrawLine(accentClr, 2.5f,
                bounds.X + 4, bounds.Y + 10, bounds.X + 4, bounds.Bottom - 10);
            ...
            // 标题
            g.DrawText("💬 需要您的输入", HeaderFont, answered ? AnsweredFg : HeaderFg,
                new Rectangle(textX, y, textW, HeaderH),
                FormatFlags.VerticalCenter | FormatFlags.Left);
            y += HeaderH + SepGap;

            // 分隔线
            g.DrawLine(answered ? AnsweredBorder : SepColor, 1f,
                textX, y, bounds.Right - SidePad, y);
            y += 1 + ContentGap;

            // 问题文本
            var qSize = g.MeasureText(Question, QuestionFont, textW, FormatFlags.Top | FormatFlags.Left);
            g.DrawText(Question, QuestionFont, QuestionFg,
            ...
            if (!answered)
            {
                ...
                for (int i = 0; i < _pillRects.Count && i < Options.Count; i++)
                {
                    ...
                    using (var path = GdiHelper.RoundedRect(pillBounds, 6))
                    {
                        g.Fill(hovered ? PillHoverBg : PillBg, path);
                        g.Draw(PillBorder, 1f, path);
                    }

                    if (Options[i].Description != null)
                    {
                        int labelH = PillH - 4;
                        g.DrawText(Options[i].Label, PillFont, PillText,
                            new Rectangle(pillBounds.X + PillPadH, pillBounds.Y + 2, pillBounds.Width - PillPadH * 2, labelH),
                            FormatFlags.VerticalCenter | FormatFlags.Left);
                        g.DrawText(Options[i].Description, PillDescFont, PillDescColor,
                            new Rectangle(pillBounds.X + PillPadH, pillBounds.Y + labelH - 2, pillBounds.Width - PillPadH * 2, 14),
                            FormatFlags.Top | FormatFlags.Left);
                    }
                    else
                    {
                        g.DrawText(Options[i].Label, PillFont, PillText, pillBounds,
                            FormatFlags.Center);
                    }
                }
            ...
            {
                // 已回答：显示答案
                g.DrawText($"已回答：{_answer}", AnsweredFont, AnsweredFg,
    ...
    //  继承 VirtualItem，由 VirtualPanel 统一管理绘制与虚拟化
    // ════════════════════════════════════════════════════════════════

    public class MessageGroup : AntdUI.VirtualItem
    {
        public ChatRole Role { get; }
        public string UserName { get; }
        public Image Avatar { get; }

        /// <summary>所属面板引用（添加到 RichChatPanel 时设置）</summary>
        internal RichChatPanel Panel { get; set; }
        ...
        public MessageGroup(string name, Image avatar, ChatRole role)
        {
            Role = role;
            UserName = name;
            Avatar = avatar;
        ...
        public void SetText(string text)
        {
            ...
            {
                // 当前文本已不包含 think，清理当前段落遗留的 ThinkBlock
                for (int i = _blocks.Count - 1; i >= sectionStart; i--)
                {
                    if (_blocks[i] is ThinkBlock)
                        _blocks.RemoveAt(i);
        ...
        internal AskUserCard AddAskUserCard(string question, List<AskUserOption> options, bool allowFreeInput)
        {
            ...
            // 如果允许自由输入，嵌入 TextBox 和提交按钮到面板
            if (allowFreeInput && Panel != null)
            {
        ...
        private static int BubbleContentWidth(int totalWidth)
        {
            int maxBubble = (int)(totalWidth * 0.80);
        ...
        {
            int totalW = e.Rect.Width > 0 ? e.Rect.Width : e.Panel.ClientSize.Width;
            int contentW = BubbleContentWidth(totalW);
            EnsureMeasureCache(g, contentW);
            int bubbleH = _cachedTotalContentHeight + BubblePadV * 2;
            return new Size(totalW, bubbleH + 6);
        }
        ...
        public override void Paint(Canvas g, VirtualPanelArgs e)
        {
            ...
            int bubbleW = Math.Max(BubbleRadius * 2 + 2, contentW + BubblePadH * 2);
            int bubbleH = Math.Max(BubbleRadius * 2 + 2, _cachedTotalContentHeight + BubblePadV * 2);
            ...
            if (isUser)
            {
                avatarX = rect.Right - AvatarSize - 6;
            ...
            if (Avatar != null)
            {
                var avatarRectF = new RectangleF(avatarX, bubbleY + 4, AvatarSize, AvatarSize);
                g.Image(avatarRectF, Avatar, TFit.Cover, AvatarSize / 2f, true);
            }

            // ── 绘制气泡阴影 ──
            var shadowRect = new Rectangle(bubbleX + 1, bubbleY + 2, bubbleW, bubbleH);
            using (var shadowPath = GdiHelper.RoundedRect(shadowRect, BubbleRadius))
                g.Fill(ShadowColor, shadowPath);

            // ── 绘制气泡背景 ──
            var bubbleRect = new Rectangle(bubbleX, bubbleY, bubbleW, bubbleH);
            using (var path = GdiHelper.RoundedRect(bubbleRect, BubbleRadius))
            {
                g.Fill(isUser ? UserBg : AIBg, path);
                if (!isUser)
                    g.Draw(AIBorder, 1f, path);
            }
        ...
        {
            int contentW = BubbleContentWidth(e.Rect.Width);

            if (TryFindBlock(e.Location, e.Rect, contentW, out var block, out var blockRect))
            {
        ...
        {
            if (_measureDirty || _cachedBlockHeights.Count == 0)
                return false;

            if (!_containsInteractiveBlocks)
            {
                sender.SetCursor(false);
                return false;
            }
        ...
        {
            foreach (var askCard in _blocks.OfType<AskUserCard>())
            {
                if (askCard.EmbeddedInput != null)
                {
                    var input = askCard.EmbeddedInput;
                    if (sender != null && sender.Controls.Contains(input))
                        sender.Controls.Remove(input);
                    input.Dispose();
                    askCard.EmbeddedInput = null;
                }

                if (askCard.EmbeddedSubmitBtn != null)
                {
                    var button = askCard.EmbeddedSubmitBtn;
                    if (sender != null && sender.Controls.Contains(button))
                        sender.Controls.Remove(button);
                    button.Dispose();
                    askCard.EmbeddedSubmitBtn = null;
                }
            }
        ...
        private void EnsureMeasureCache(Canvas g, int contentW)
        {
        ...
        private bool TryFindBlock(Point location, Rectangle itemRect, int contentW, out IContentBlock hitBlock, out Rectangle hitRect)
        {
            hitBlock = null;
            ...
            int bubbleX = isUser
                ? itemRect.Right - AvatarSize - 6 - AvatarGap - bubbleW
                : itemRect.X + 6 + AvatarSize + AvatarGap;

            int blockX = bubbleX + BubblePadH;
            int blockY = itemRect.Y + BubblePadV;
    ...
    // ════════════════════════════════════════════════════════════════
    //  RichChatPanel — 富消息聊天面板（VirtualPanel 虚拟化容器）
    // ════════════════════════════════════════════════════════════════

    public class RichChatPanel : AntdUI.VirtualPanel
    {
        ...
        {
            var msg = new MessageGroup(name, avatar, role) { Panel = this };
            if (!string.IsNullOrEmpty(text))
                msg.SetText(text);
            Items.Add(msg);
            ScrollToEnd();
        ...
        public MessageGroup LastAIMessage
        {
            get
            {
                for (int i = Items.Count - 1; i >= 0; i--)
                    if (Items[i] is MessageGroup mg && mg.Role == ChatRole.AI) return mg;
                return null;
            }
        }
        ...
        public MessageGroup LastMessage
        {
            ...
            {
                for (int i = Items.Count - 1; i >= 0; i--)
                    if (Items[i] is MessageGroup mg) return mg;
                return null;
            }
        }
        ...
        public void ScrollToEnd()
        {
            ...
            ScrollBar.ValueY = ScrollBar.MaxY;
        }
    }
}
