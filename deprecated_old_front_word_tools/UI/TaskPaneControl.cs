    ...
    public partial class TaskPaneControl : UserControl, NetOffice.WordApi.Tools.ITaskPane
    {
        ...
        private async void ProcessAssistantMessage(string userMessage, string attachedSelectionText, RichChatPanel panel)
        {
            ...
            catch (Exception ex)
            {
                try
                {
                    DebugLogger.Instance.LogError("ProcessAssistantMessage", ex);
                    System.Diagnostics.Debug.WriteLine($"ProcessAssistantMessage exception: {ex}");
                    ...
                    string errorText = $"抱歉，处理消息时发生异常：{ex.Message}";
                    if (aiMsg == null)
                    {
                        var aiAvatar = GetAIAvatar();
                        aiMsg = panel?.AddMessage("AI福星", aiAvatar, ChatRole.AI, errorText);
                    }
                    else
                    {
                        aiMsg.SetText(errorText);
                    }
                }
                catch (Exception nested)
                {
                    System.Diagnostics.Debug.WriteLine($"ProcessAssistantMessage fallback exception: {nested}");
                }
            }
            finally
            {
