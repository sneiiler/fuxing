...
namespace FuXingAgent.Core
{
    ...
    public class DocumentGraphBuilder
    {
        ...
        /// <summary>深度路径：全量段落提取 + LLM 推断标题</summary>
            ...
            CancellationToken cancellation = default)
        {
            ...
            var astRoot = await BuildAstAsync(rawStructure, subAgentRunner, cancellation);
        ...
        /// <summary>展开一个 TextBlock 节点为子段落</summary>
        public void ExpandTextBlock(Word.Document doc, DocumentGraph graph, string textBlockNodeId)
        {
        ...
        public static HeadingInfo FindHeading(Word.Document doc, string headingName)
        {
            foreach (Word.Paragraph para in doc.Paragraphs)
            {
        ...
        public static int FindSectionEnd(Word.Document doc, Word.Paragraph targetPara, int targetLevel)
        {
        ...
        public static (int Start, int End) GetSectionRange(
            Word.Document doc, Word.Paragraph targetPara, int targetLevel, bool includeHeading)
        {
        ...
        internal static Word.Range GetNodeRange(Word.Document doc, DocNode node)
        {
            if (node.Meta != null
        ...
        private static List<HeadingEntry> ExtractHeadings(Word.Document doc)
        {
        ...
        private static string ExtractBodyPreview(Word.Range bodyRange)
        {
        ...
        private static DocNode CreateRootNode(Word.Document doc)
        {
            return new DocNode
            {
                Id = "doc",
        ...
        private void BuildPreamble(Word.Document doc, DocumentGraph graph, DocNode root, List<HeadingEntry> headings)
        {
        ...
        private void DiscoverAllSectionContents(
            Word.Document doc, DocumentGraph graph,
            IProgress<(float ratio, string message)> progress = null)
        {
            ...
            for (int idx = 0; idx < total; idx++)
            {
                ...
                // 进度区间：0.15 ~ 0.95，按章节数等分
                float ratio = 0.15f + 0.80f * idx / Math.Max(total, 1);
                progress?.Report((ratio,
                    $"分析章节 {idx + 1}/{total}：{Ellipsis(section.Title, 16)}（{graph.Index.Count} 节点）"));
        ...
        private void CreateHeadingNode(Word.Document doc, DocumentGraph graph, DocNode section)
        {
            ...
            List<DocNode> outContentNodes)
        {
            ...
            var textBlockParas = new List<Word.Paragraph>();
            var listParas = new List<Word.Paragraph>();
            bool inList = false;
            ...
            foreach (Word.Paragraph para in doc.Paragraphs)
            {
                ...
                try
                {
                    isInTable = (bool)paraRange.get_Information(Word.WdInformation.wdWithInTable);
                }
                catch { }
        ...
        private void FlushTextBlock(Word.Document doc, DocumentGraph graph,
            DocNode parent, List<Word.Paragraph> paras, List<DocNode> outNodes)
        {
        ...
        private void FlushList(Word.Document doc, DocumentGraph graph,
            DocNode parent, List<Word.Paragraph> paras, List<DocNode> outNodes)
        {
            if (paras.Count == 0) return;
        ...
        private DocNode CreateImageNode(Word.Document doc, DocumentGraph graph,
            DocNode parent, Word.Range paraRange)
        {
        ...
        private void DiscoverTables(Word.Document doc, DocumentGraph graph,
        ...
        private static void MergeContentNodesIntoSection(
            Word.Document doc, DocumentGraph graph, DocNode section, List<DocNode> contentNodes)
        {
        ...
        private static DeepStructure ExtractDeepStructure(Word.Document doc)
        {
            var sw = Stopwatch.StartNew();
            ...
            foreach (Word.Paragraph para in doc.Paragraphs)
            {
                ...
                string styleName = "";
                try { styleName = ((dynamic)para.get_Style()).NameLocal?.ToString() ?? ""; }
                catch { }
                ...
                try
                {
                    ...
                    if (listFormat.ListType != Word.WdListType.wdListNoNumbering)
                    {
                        isListItem = true;
                ...
                try
                {
                    isInTable = (bool)range.get_Information(Word.WdInformation.wdWithInTable);
                }
                catch { }
            ...
            CancellationToken cancellation = default)
        {
            ...
            if (nonBlankCount <= 3)
            {
                Debug.WriteLine($"[GraphBuilder] 仅 {nonBlankCount} 个非空段落，跳过 LLM 推断");
            ...
            else
            {
                ...
                headings = await InferHeadingsWithLlm(
                    rawStructure, outlineLevelHints, singleLineCandidates,
                    subAgentRunner, cancellation);
            }
            ...
            DeepStructure structure, HashSet<int> hintedParaIndices)
        {
            ...
            foreach (var p in structure.Paragraphs)
            {
                if (p.IsBlank || p.IsInTable || p.IsListItem) continue;
                if (hintedParaIndices.Contains(p.Index)) continue;
                if (p.Text == null || p.Text.Length > SingleLineCandidateMaxLength || p.Text.Length < 2) continue;
        ...
        private static string BuildCandidateContext(DeepStructure structure, int paraIndex)
        {
            ...
            {
                if (!structure.Paragraphs[i].IsBlank) { prevPara = structure.Paragraphs[i]; break; }
            }

            ParaMeta nextPara = null;
            for (int i = paraIndex; i < structure.Paragraphs.Count; i++)
            {
                if (!structure.Paragraphs[i].IsBlank) { nextPara = structure.Paragraphs[i]; break; }
            }

            sb.AppendLine(prevPara != null
                ? $"  前文: {prevPara.ToDescriptionLine()}"
                : "  前文: (文档开头)");
            sb.AppendLine($"  ★候选: {para.ToDescriptionLine()}");
            sb.AppendLine(nextPara != null
                ? $"  后文: {nextPara.ToDescriptionLine()}"
                : "  后文: (文档末尾)");
            ...
            CancellationToken cancellation)
        {
            ...
            string systemPrompt = "你是一个文档标题层级推断专家。根据提供的段落格式特征（字号、加粗、对齐等）和大纲级别提示，判断哪些段落是标题，以及它们的层级。直接输出结果，不要解释过程。";

            string result = await subAgentRunner.RunAsync(
            ...
            var headings = ParseHeadingList(result);

            if (headings.Count == 0)
                Debug.WriteLine("[GraphBuilder] LLM 未返回任何标题");
            else
        ...
        private static List<InferredHeading> ParseHeadingList(string llmOutput)
        {
            ...
            catch (Newtonsoft.Json.JsonException ex)
            {
                throw new InvalidOperationException(
                    ...
                    $"提取到的内容: {json.Substring(0, Math.Min(200, json.Length))}", ex);
            }
            ...
            foreach (var item in arr)
            {
                ...
                if (paraIndex <= 0 || level < 1 || level > 6) continue;
                if (!seen.Add(paraIndex)) continue;
        ...
        private static string ExtractJsonArray(string text)
        {
            ...
            if (codeStart >= 0)
            {
                ...
                if (start >= 0 && end > start)
                {
                    ...
                    if (LooksLikeJsonArray(candidate)) return candidate;
                }
            }
            ...
            DeepStructure structure, List<InferredHeading> headings)
        {
            ...
            if (headings.Count == 0) return root;
        ...
        private enum AstNodeType { Root, Section }
        ...
        private class ParaMeta
        {
            ...
            public string ToDescriptionLine()
            {
                ...
                if (fontParts.Count > 0) tags.Add($"字体:{string.Join(", ", fontParts)}");

                if (!string.IsNullOrEmpty(StyleName)) tags.Add($"样式:{StyleName}");
                if (OutlineLevel >= 1 && OutlineLevel <= 9) tags.Add($"大纲:{OutlineLevel}级");
                if (IsListItem) tags.Add($"列表:L{ListLevel}");
                if (IsInTable) tags.Add("表格内");
