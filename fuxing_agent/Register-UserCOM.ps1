...
$AssemblyName  = "FuXingAgent, Version=3.0.0.0, Culture=neutral, PublicKeyToken=null"
...
function Register-ComClass {
    ...
    foreach ($clsidRoot in $clsidRoots) {
        ...
        New-Item -Path "$clsidRoot\Control" -Force | Out-Null
    }
...
# 1. Connect (Add-in 入口)
Register-ComClass -CLSID "{D0E1F2A3-B4C5-6789-ABCD-EF0123456789}" `
                  -ProgId "FuXingAgent.Connect" -ClassName "FuXingAgent.Connect" `
                  -CodeBase $CodeBase -AssemblyName $AssemblyName `
                  -RuntimeVer $RuntimeVer -ManagedCATID $ManagedCATID

# 2. TaskPaneHost (CustomTaskPane 控件)
Register-ComClass -CLSID "{E1F2A3B4-C5D6-7890-ABCD-EF1234567890}" `
                  -ProgId "FuXingAgent.TaskPaneHost" -ClassName "FuXingAgent.TaskPaneHost" `
                  -CodeBase $CodeBase -AssemblyName $AssemblyName `
                  -RuntimeVer $RuntimeVer -ManagedCATID $ManagedCATID

# 3. Microsoft Word Add-in
$addinKey = "HKCU:\Software\Microsoft\Office\Word\Addins\FuXingAgent.Connect"
...
# 4. WPS Add-in
$wpsAddinKey = "HKCU:\Software\Kingsoft\Office\WPS\Addins\FuXingAgent.Connect"
...
New-ItemProperty -Path $wpsWL -Name "FuXingAgent.Connect" -Value "" -PropertyType String -Force | Out-Null
